...
void PlayerShip::HandleInput(const InputState& input)
{
	...
	{
		Vector2 direction;
		if (input.IsKeyDown(Key::DOWN)) direction.Y+=5;
		if (input.IsKeyDown(Key::UP)) direction.Y-=5;
		if (input.IsKeyDown(Key::Right)) direction.X += 5;
		if (input.IsKeyDown(Key::Left)) direction.X-=5;
